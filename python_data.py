...
def multiple_letter_count(phrase):
    ...
    count_ltr = {}
    for ltr in phrase:
       count_ltr[ltr] = count_ltr.get(ltr,0) + 1 
    return count_ltr
...
def sum_floats(nums):
    ...
    flt_num = [num for num in nums if isinstance(num,float) == True]
    return sum(flt_num)
...
def vowel_count(phrase):
    ...
    for vow in phrase:
       if vow in vowels:
        count_vow[vow] = count_vow.get(vow,0) + 1 
    return count_vow
def titleize(phrase):
...
def two_list_dictionary(keys, values):
   ...
   """
    ...
    return {dict.fromkeys(k,v) for k,v in (keys,values)}
...
def two_oldest_ages(ages):
    ...
    # from it.
    tup_ages = []
    sort_age = sorted(set(ages))
    tup_ages.append(max(sort_age))
    sort_age.pop(-1)
    tup_ages.append(max(sort_age))
    return tuple(sorted(tup_ages))
